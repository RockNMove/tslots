...
WITH
    -- Дневной агрегат движений по зерну (слот, агент, товар, день).
    -- real_in/out и move_in/out уже разделены в int_premart__operations_each.
    -- Идёт первым — grain и grid выводятся из него, чтобы не сканировать исходник дважды.
    daily_agg AS (
        SELECT
            agent_id
            , slot_id
    ...
    -- Уникальные тройки зерна (слот, агент, товар) и дата первой операции.
...
LEFT JOIN {{ ref('int_enrich__slots_extended') }}         s ON b.slot_id  = s.slot_id
LEFT JOIN {{ ref('int_enrich__items_united_extended') }}  i ON b.item_id  = i.item_id
WHERE b.is_used != 0
